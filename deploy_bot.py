...
TABLE_attack_session = """CREATE TABLE attack_session(
                          id serial PRIMARY KEY,
                          user_id BIGINT references users(id),
                          session_date DATE,
                          type_of_attack varchar(80),
                          melee_weapon varchar(80),
                          ranged_weapon varchar(80),
                          fast_or_hard_1 varchar(80),
                          fast_or_hard_2 varchar(80),
                          fast_or_hard_3 varchar(80),
...
TABLE_multiple_modifier = """CREATE TABLE mod ( 
                             id serial PRIMARY KEY, 
                             session_id INT references attack_session(id),
                             value varchar(80)
                             );"""
    
TABLE_multiple_effects = """CREATE TABLE effects ( 
                             id serial PRIMARY KEY, 
                             session_id INT references attack_session(id),
                             value varchar(80)
                             );"""

TABLE_critical_injury = """CREATE TABLE critical_injury ( 
...
TABLE_defense_session ="""CREATE TABLE defense_session(
                          id serial PRIMARY KEY,
                          user_id BIGINT references users(id),
                          session_date DATE,
...
TABLE_multiple_d_modifier = """CREATE TABLE d_mod ( 
                             id serial PRIMARY KEY, 
                             session_id INT references defense_session(id),
                             value varchar(80)
                             );"""
    
TABLE_multiple_d_effects = """CREATE TABLE d_effects ( 
                             id serial PRIMARY KEY, 
                             session_id INT references defense_session(id),
                             value varchar(80)
                             );"""
...
with psycopg2.connect(
    ...
    with conn.cursor(cursor_factory=DictCursor) as cur:
        ...
        cur.execute(SQL(TABLE_multiple_d_effects))
